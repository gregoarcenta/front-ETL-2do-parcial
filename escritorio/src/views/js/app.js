...
async function indexCitaFetch() {
  const data = await fetch(`http://localhost:5000/datamart-data`);
  const result = await data.json();
  dataRender(result);
}

async function syncDatamart() {
  const response = await fetch(`http://localhost:5000/sync-motel`);
...
//   console.log(data.result);
  ...
  if (tbody) {
    ...
    data.result.forEach((servicio) => {
      const date = new Date(servicio.fecha_visita).toUTCString();
      const newArray = date.split(" ")
      const newStringDate = `${newArray[0]} ${newArray[1]} ${newArray[2]} ${newArray[3]}`
      templateHtmlTable += ` 
      <tr>
         <td class="py-3">${servicio.nombre_servicio}</th>
         <td class="py-3">${newStringDate}</td>
         <td class="py-3">${servicio.pon_calificacion}</td>
         <td class="py-3">${servicio.desc_calificacion}</td>
      </tr>
      `;
    });
